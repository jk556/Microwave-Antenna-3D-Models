...
for i = 1:floor(2e3*Lambda/(2*r + d))
    name = string("cylinder"+i);
    name_ = string("cylinder_"+i);
    hfssCylinder(fid, name, 'Y', [-a/2, -b/2, -1e3*Lambda + (2*i-1)*r + (i-1)*d], r, b, 'm', material_air);
    hfssSubtract(fid, 'Air', name, 'false');
    hfssCylinder(fid, name_, 'Y', [a/2, -b/2, -1e3*Lambda + (2*i-1)*r + (i-1)*d], r, b, 'm', material_air);
...
for i = 1:floor(2e3*Lambda/(2*r + d))
    ...
    hfssCylinder(fid, name, 'Y', [-a/2, -b/2, -1e3*Lambda + (2*i-1)*r + (i-1)*d], r, b, 'm', material_semi);

    hfssCylinder(fid, name_, 'Y', [a/2, -b/2, -1e3*Lambda + (2*i-1)*r + (i-1)*d], r, b, 'm', material_semi);
end
